...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $reportTitle = htmlspecialchars($_POST['reportTitle']);  // Sanitize input to prevent XSS
    $reportContent = htmlspecialchars($_POST['reportContent']); // Sanitize input
    $username = $_SESSION['username']; // Use logged-in user's username
    $status = $_POST['report_status']; // Default status when creating a new report
    ...
    // Prepare and bind
    $stmt = $db->prepare("INSERT INTO reports (report_title, report_description, submitted_by, report_status, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())");
    $stmt->bind_param('ssss', $reportTitle, $reportContent, $username, $status);

    // Execute query
    if ($stmt->execute()) {
        // Redirect to report overview page after saving
        header("Location: report.php?page=overview");
        exit();
    } else {
...
<body>
    ...
    <div class="content" id="content">
        ...
        <div class="card">
            <div class="card-header">New Report Form</div>
            <div class="card-body">
                <form method="POST" action="new-report.php">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="reportContent" name="reportContent" rows="5" required></textarea>
                    </div>
                    <div class="form-group">
                        <!-- The username is automatically handled by session -->
